...
- Use this as a Codex workflow skill; `/graphify` means invoke the `graphify` CLI or follow the documented steps from the current project root.
- On Windows, translate Bash examples to PowerShell and use an explicitly available Python interpreter if `python3` is unavailable.
- If `graphify` is not installed, install it only after confirming the target environment and package manager; otherwise explain the missing dependency.
