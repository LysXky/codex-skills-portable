...
- Resolve relative files from this skill directory under `C:\Users\Felipe.TapiaP\.codex\skills` or `$CODEX_HOME/skills`.
- Treat Claude slash commands as named Codex workflows; use regular user questions instead of `AskUserQuestion`.
- On Windows, translate Bash snippets to PowerShell and prefer an explicitly available Python interpreter over the Microsoft Store `python` alias when needed.
- Verify any optional browser, MCP, image-generation, or external AI service before relying on it.
