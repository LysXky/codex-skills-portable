...
```bash
# Check key last used
aws iam get-access-key-last-used --access-key-id EXAMPLE_AWS_ACCESS_KEY_REDACTED

# List access keys for user
aws iam list-access-keys --user-name jsmith

# Deactivate exposed key
aws iam update-access-key --access-key-id EXAMPLE_AWS_ACCESS_KEY_REDACTED \
  --user-name jsmith --status Inactive

# Delete key
aws iam delete-access-key --access-key-id EXAMPLE_AWS_ACCESS_KEY_REDACTED \
  --user-name jsmith
...
WHERE useridentity.accesskeyid = 'EXAMPLE_AWS_ACCESS_KEY_REDACTED'
